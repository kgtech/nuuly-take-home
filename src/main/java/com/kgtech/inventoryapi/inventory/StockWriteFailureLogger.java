...
/** Logs the SKU when a stock write fails for good (W2). skuId is the first argument of every service write. */
@Component
class StockWriteFailureLogger {
    ...
    void onRetryAborted(MethodRetryEvent event) {
        ...
        if (serializationFailure.shouldRetry(event.getMethod(), event.getFailure())) {
            ...
            log.error("Stock write failed without retry for SKU {}", skuId, event.getFailure());
        }
    }
}
